	...
	if gatewayMode {
		...
		if *configURL != "" {
			...
			if _, err := os.Stat(*configPath); os.IsNotExist(err) {
				...
				logger.Printf("")
				logger.Printf("📝 To add APIs, use one of these methods:")
				logger.Printf("")
				logger.Printf("1️⃣  Web UI (Easiest - Recommended)")
				logger.Printf("   Open: http://localhost:19190/ui/")
				logger.Printf("   (Automatically started with the service)")
				logger.Printf("")
				logger.Printf("2️⃣  Config File")
				logger.Printf("   Edit: %s", *configPath)
				logger.Printf("   See examples: https://github.com/emadomedher/skyline-mcp/tree/main/examples")
				logger.Printf("")
				logger.Printf("📖 Documentation: https://skyline.projex.cc/docs")
				logger.Printf("")
