...
func init() {
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  skyline gateway start       Start server in the background\n")
		...
		fmt.Fprintf(os.Stderr, "  skyline auth login          Authenticate with Skyline Cloud (--api-key flag or interactive)\n")
		fmt.Fprintf(os.Stderr, "  skyline auth status         Show cloud account and tunnel usage\n")
		fmt.Fprintf(os.Stderr, "  skyline auth logout         Remove stored API key\n")
		fmt.Fprintf(os.Stderr, "  skyline update              Update Skyline to the latest version\n\n")
		fmt.Fprintf(os.Stderr, "Cloud:\n")
		fmt.Fprintf(os.Stderr, "  When authenticated, 'gateway start' automatically connects to Skyline Cloud\n")
		fmt.Fprintf(os.Stderr, "  via an outbound WebSocket tunnel. Without an API key, runs standalone.\n\n")
		fmt.Fprintf(os.Stderr, "Examples:\n")
		fmt.Fprintf(os.Stderr, "  # Start server in the background\n")
		fmt.Fprintf(os.Stderr, "  skyline gateway start\n\n")
		fmt.Fprintf(os.Stderr, "  # Start HTTP server in the foreground (default)\n")
