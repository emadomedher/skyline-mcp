...
- Domain: `*.example.com` (already configured)
...
**Example token:** `REDACTED_BEARER_TOKEN_1`
...
```yaml
stringData:
  bearer-token: "REDACTED_BEARER_TOKEN_1"
```
...
# Test locally first
curl -X POST https://skyline.example.com/mcp \
...
2. **Add Skyline Server:**
   ...
   - **Server URL:** `https://skyline.example.com/mcp`
   - **Authentication:** Bearer token
   - **Token:** `REDACTED_BEARER_TOKEN_1`
...
- **Wildcard cert:** `*.example.com` (if already configured)
...
- Test GraphQL endpoint: `curl https://gitlab.example.com/api/graphql`
...
ingress:
  - hostname: skyline.example.com
    service: http://localhost:8089
    originRequest:
      httpHostHeader: skyline.example.com
