...
3. **Random subdomain** - Uses unpublished `<random>.medher.online` for security
...
- Wildcard DNS for `*.medher.online` pointing to your cluster
...
This will:
- Generate a random subdomain (e.g., `f4a8c2e1b9d7f3e6a8c2b4d7e9f1a3c5.medher.online`)
...
| **dinosaurs-swagger** | Swagger 2.0 | Bearer: `dino-token` | List, Create, Get, Update, Delete dinosaurs |
| **plants-soap** | WSDL/SOAP | Bearer: `mock-token` | SOAP operations for plants |
| **cars-graphql** | GraphQL | Basic: `graphql-user` / `graphql-pass` | Query and mutate cars |
...
```
URL: https://<random>.medher.online/mcp
...
echo "Subdomain: ${RANDOM_SUBDOMAIN}.medher.online"
...
```bash
curl -X POST https://<random>.medher.online/mcp \
...
ChatGPT Web (https://chatgpt.com)
    ...
    ↓ HTTPS + Bearer Token
    ↓
Random Subdomain (*.medher.online)
    ↓
